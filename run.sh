...
function show_help {
    ...
    echo "  ./run.sh --action preprocess --dataset /path/to/DFC2023mini"
    echo "  "
    echo "  # Train on dataset (automatically uses NPY if available)"
    echo "  ./run.sh --action train --dataset /path/to/DFC2023mini --patience 20"
    echo "  "
    echo "  # Train with specific batch size (useful for large images or limited GPU memory)"
    echo "  ./run.sh --action train --dataset /path/to/DFC2023mini --batch-size 2"
    echo "  "
    echo "  # Train with custom learning rate"
    echo "  ./run.sh --action train --dataset /path/to/DFC2023mini --learning-rate 1e-4"
    echo "  "
    echo "  # Resume training from last checkpoint"
    echo "  ./run.sh --action train --dataset /path/to/DFC2023mini --resume weights/DFC2023mini/last.ckpt"
    echo "  "
    echo "  # Predict on dataset (automatically uses NPY if available)"
    echo "  ./run.sh --action predict --dataset /path/to/DFC2023mini --weights weights/best.ckpt"
    echo "  "
    echo "  # Evaluate predictions against ground truth"
    echo "  ./run.sh --action evaluate --dataset /path/to/DFC2023mini"
    echo "  "
